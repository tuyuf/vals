...
const EMOJIS = [
    { emoji: "💖", top: "6%", right: "5%", rotate: 14, size: "2.8rem" },
    { emoji: "🥰", top: "18%", left: "4%", rotate: -12, size: "2.5rem" },
    { emoji: "🌙", bottom: "22%", right: "4%", rotate: 18, size: "2.3rem" },
    { emoji: "🎆", bottom: "10%", left: "5%", rotate: -16, size: "2.4rem" },
    { emoji: "💐", top: "42%", right: "3%", rotate: 10, size: "2.2rem" },
    { emoji: "🫶", top: "55%", left: "2%", rotate: -8, size: "2.6rem" },
];
...
function getMoodLabel(val: number): string {
    if (val <= 20) return "Capek tapi seneng";
    if (val <= 40) return "Lumayan seru";
    if (val <= 60) return "Seneng banget!";
    if (val <= 80) return "Best date ever!";
    return "Gak mau pulang 🥺";
}

export default function MoodAfterDate() {
    const { setMoodAfter, nextStep } = useQuestStore();
